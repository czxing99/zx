...
public class SqlServerController {
    ...
    public ModelAndView  index(){
        List<TableName> tableList = sqlServerService.getAll();

        ModelAndView m = new ModelAndView();
        m.addObject("root","zx");
        m.addObject("page",tableList);
        m.setViewName("/autoCoder/index");
        return m ;
    }
    @RequestMapping("/getColumnName")
    @ResponseBody
    public List<ColumnName> getColumnName(@Param("tableName") String tableName){
        List<ColumnName> lists = sqlServerService.getTableColumn(tableName);
       ...
       Map  map = new HashMap<String,String>();
        ...
        try {
            Head head = Head.builder().baseUrl("com.zx").
                    ...
                    .memo("autocoding")
                            .build();
            FreemarkUtil.setRoot("sqlserver");
